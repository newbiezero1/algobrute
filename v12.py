...
def test(cache, ohlc, ifilterEma, islowEma, filterEmaKoridor, takeProfit, stopLoss, name='15'):
...
def run_test(params):
    cache, ohlc, filter_ema, slow_ema, filterEmaKoridor, takeProfit, stopLoss = params
    report = test(cache, ohlc[-15000:], filter_ema, slow_ema, filterEmaKoridor, takeProfit, stopLoss, '15')
    with open('log.txt', "w") as file:
        file.write(f'{filter_ema} {slow_ema} {filterEmaKoridor} {takeProfit} {stopLoss} {report["Net Profit"]}\n')
    if report['Net Profit'] < 0:
        return {}
    report30 = test(cache, ohlc[-30000:], filter_ema, slow_ema, filterEmaKoridor, takeProfit, stopLoss, '30')
    report45 = test(cache, ohlc, filter_ema, slow_ema,  filterEmaKoridor, takeProfit, stopLoss, '45')
    report['Net Profit 30k'] = report30['Net Profit']
    report['Net Profit 45k'] = report45['Net Profit']
    report['params'] = f'{filter_ema} {slow_ema} {filterEmaKoridor} {takeProfit} {stopLoss}'
    return report
...
if __name__ == "__main__":
    ...
    for coin in coins:
        for tf in tfs:
            ...
            filter_ema_range = range(200, 300)
            slow_ema_range = range(100, 145)
            filterEmaKoridor_range = np.arange(0.5, 3.25, 0.25)
            takeProfit_range = np.arange(1.0, 11.0, 0.5)
            stopLoss_range = np.arange(1.0, 11.0, 0.5)

            param_combinations = [
                (cache, ohlc, filter_ema,slow_ema, filterEmaKoridor, takeProfit, stopLoss)
                for filter_ema in filter_ema_range
                for slow_ema in slow_ema_range
                for filterEmaKoridor in filterEmaKoridor_range
