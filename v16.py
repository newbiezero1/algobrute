...
def test(ohlc, ifilterEma, islowEma, filterEmaKoridor, takeProfit, stopLoss, name='15'):
...
def run_test(params):
    ohlc, filter_ema, slow_ema, filterEmaKoridor, takeProfit, stopLoss = params
    report = test(ohlc[-15000:], filter_ema, slow_ema, filterEmaKoridor, takeProfit, stopLoss, '15')
    with open('log.txt', "w") as file:
        file.write(f'{filter_ema} {slow_ema} {filterEmaKoridor} {takeProfit} {stopLoss} {report["Net Profit"]}\n')
    if report['Net Profit'] < 0:
        return {}
    report30 = test(ohlc[-30000:], filter_ema, slow_ema, filterEmaKoridor, takeProfit, stopLoss, '30')
    report45 = test(ohlc, filter_ema, slow_ema,  filterEmaKoridor, takeProfit, stopLoss, '45')
...
if __name__ == "__main__":
    ...
    for coin in coins:
        for tf in tfs:
            ...
            param_combinations = [
                (ohlc, filter_ema,slow_ema, filterEmaKoridor, takeProfit, stopLoss)
