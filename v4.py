...
def test(cache, ohlc, rsi_length, overbuy, oversell, takeProfit, stopLoss, name='15'):
...
def run_test(params):
    cache, ohlc, rsi_length, overbuy, oversell, takeProfit, stopLoss = params
    report = test(cache, ohlc[-15000:], rsi_length, overbuy, oversell, takeProfit, stopLoss, '15')
    ...
    if report['Net Profit'] < 0:
        return {}
    report30 = test(cache, ohlc[-30000:],rsi_length, overbuy, oversell, takeProfit, stopLoss, '30')
    report45 = test(cache, ohlc, rsi_length, overbuy, oversell, takeProfit, stopLoss, '45')
...
if __name__ == "__main__":
    ...
    for coin in coins:
        for tf in tfs:
            ...
            ta.save_sorted_final_report_to_csv(report_history, f'res/v4_{coin}_{tf}.csv')
            ta.save_sorted_filtered_final_report_to_csv(report_history, f'res/v4_filtered_{coin}_{tf}.csv')
            end_time = time.time()
