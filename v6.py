...
def test(cache, ohlc, ifilterEma, ifastEma, islowEma, takeProfit, stopLoss, name='15'):
...
def run_test(params):
    cache, ohlc, filter_ema,fast_ema, slow_ema, takeProfit, stopLoss = params
    report = test(cache, ohlc[-15000:], filter_ema, fast_ema, slow_ema, takeProfit, stopLoss, '15')
    with open('log.txt', "w") as file:
        file.write(f'{filter_ema} {fast_ema} {slow_ema} {takeProfit} {stopLoss} {report["Net Profit"]}\n')
    if report['Net Profit'] < 0:
        return {}
    report30 = test(cache, ohlc[-30000:], filter_ema, fast_ema, slow_ema, takeProfit, stopLoss, '30')
    report45 = test(cache, ohlc, filter_ema, fast_ema, slow_ema, takeProfit, stopLoss, '45')
...
if __name__ == "__main__":
    ...
    for coin in coins:
        for tf in tfs:
            ...
            param_combinations = [
                (cache, ohlc, filter_ema, fast_ema, slow_ema, takeProfit, stopLoss)
